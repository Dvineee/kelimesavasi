...
import { UserProfile, GameRoomData } from '../types.ts';
    ...
    <div className="max-w-6xl mx-auto w-full flex flex-col gap-4 md:gap-6 animate-in fade-in slide-in-from-bottom-6 duration-700 pb-10">
          ...
          {activeTab === 'rooms' ? (
            <div className="glass-card rounded-[2.5rem] p-4 md:p-8 min-h-[400px]">
              ...
              <div className="space-y-3">
                {rooms.map((room, idx) => (
                  <div key={room.id} className="group flex items-center justify-between p-4 md:p-5 bg-slate-900/30 rounded-3xl border border-white/5 hover:border-orange-500/20 hover:bg-slate-900/60 transition-all">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-500 uppercase font-black tracking-widest mt-0.5">
                          {room.owner} <span className="mx-1 text-slate-800">•</span> {room.playerCount}/{room.maxPlayers}
                        </p>
                      </div>
                    </div>
                    <button onClick={() => onStartGame('ranked')} className="px-6 py-2.5 rounded-xl bg-slate-800 hover:bg-orange-600 text-white text-[10px] font-black uppercase tracking-widest transition-all">KATIL</button>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="glass-card rounded-[2.5rem] p-6 border-white/5">
            ...
            <div className="space-y-4 h-64 overflow-y-auto no-scrollbar relative">
              {[
                ...
                <div key={i} className="flex gap-3 text-[11px] animate-in slide-in-from-right-2">
                  ...
                  <div>
                    ...
                    <p className="text-[9px] text-slate-600 font-bold mt-0.5">{item.time}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
          <div className="w-full max-w-md glass-card p-8 rounded-[3rem] shadow-2xl border-white/10 relative">
            <button onClick={() => setShowCreateModal(false)} className="absolute top-6 right-6 text-slate-500 hover:text-white transition-colors">
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
            </button>
            <h2 className="text-3xl font-black font-orbitron text-white mb-2 neon-text-orange">ODA KUR</h2>
            <form onSubmit={handleCreateRoom} className="space-y-6 mt-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Oda İsmi</label>
                <input autoFocus required value={roomName} onChange={(e) => setRoomName(e.target.value)} placeholder="Efsanevi Savaş" className="w-full bg-slate-950 border border-white/10 rounded-2xl px-6 py-4 text-white outline-none focus:border-orange-500 transition-all font-bold" />
              </div>
              <button type="submit" className="w-full py-5 rounded-[1.5rem] bg-orange-600 hover:bg-orange-500 text-white font-black uppercase text-sm tracking-[0.2em] shadow-2xl shadow-orange-900/40 transition-all active:scale-95">SAVAŞI BAŞLAT</button>
            </form>
          </div>
        </div>
      )}
